...
We address the challenge of estimating the effect along a specific pathway (e.g., $A \to M \to Y$) in the simultaneous presence of treatment-induced mediator-outcome confounders $D$ and general unmeasured confounders $U$ (directly affect $A$, $D$, $M$ and $Y$).
...
2. Nonparametric estimation (Python):
   implements the proximal debiased machine learning in the cross-fitting procedure, using regularized minimax-based optimization in a simultaneous estimation strategy to solve for nested bridge functions without model specification.
