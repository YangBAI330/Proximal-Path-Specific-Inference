source('bridges.R')
###### Experiment 1: All bridge functions are correctly specified #####
...
fun_boots <- function(j) {
  ...
  psiboots_PQR[j] <<- mean( AA * ( 1+ exp(- cbind(1,ZZ,XX)%*%q0par ) ) * ( cbind(1,WW,DD,XX)%*%h1par - cbind(1,WW,XX) %*% h0par ) 
                      + (1-AA) * ( 1+ exp(- cbind(1,ZZ,XX)%*%q0par ) ) * exp( cbind(1,ZZ,DD,XX)%*%q1par ) * ( cbind(1,WW,MM,DD,XX)%*%h2par - cbind(1,WW,DD,XX)%*%h1par )
                      + AA * ( 1+ exp(- cbind(1,ZZ,XX)%*%q0par ) ) * exp( cbind(1,ZZ,DD,XX)%*%q1par ) * exp( cbind(1,ZZ,MM,DD,XX)%*%q2par ) * ( YY - cbind(1,WW,MM,DD,XX)%*%h2par )
                      + cbind(1,WW,XX) %*% h0par )
...
for (i in 1:nsimu) {
  ...
  #### proximal quadruply robust ####
  psi_PQR[i] <- mean( A * ( 1+ exp(- cbind(1,Z,X)%*%q0par ) ) * ( cbind(1, W, D, X)%*%h1par - cbind(1,W,X) %*% h0par ) 
                     + (1-A) * ( 1+ exp(- cbind(1,Z,X)%*%q0par ) ) * exp( cbind(1,Z,D,X)%*%q1par ) * ( cbind(1, W, M, D, X)%*%h2par - cbind(1, W, D, X)%*%h1par )
                     + A * ( 1+ exp(- cbind(1,Z,X)%*%q0par ) ) * exp( cbind(1,Z,D,X)%*%q1par ) * exp( cbind(1,Z,M,D,X)%*%q2par ) * ( Y - cbind(1, W, M, D, X)%*%h2par )
                     + cbind(1,W,X) %*% h0par )
