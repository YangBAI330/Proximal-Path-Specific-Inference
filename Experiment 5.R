source('bridges.R')
###### Experiment 5: h1, h0, and q2 are misspecified #####
...
fun_boots <- function(j) {
  ...
  h1par <- optim(par=inioptim,
                 fn=bridgeh1_e5, parah2=h2par, data1=data,
                 method = "BFGS", hessian=FALSE)$par
  inioptim <- c(0,0,0,0)
  h0par <- optim(par=inioptim,
                 fn=bridgeh0_e5, parah1=h1par, data1=data,
                 method = "BFGS", hessian=FALSE)$par
  ...
  q2par <- optim(par=inioptim,
                 fn=bridgeq2_e5, paraq0=q0par, paraq1=q1par, data1=data,
                 method = "BFGS", hessian=FALSE)$par
  
  psiboots_PIPW[j] <<- mean( AA*YY * ( 1+ exp(- cbind(1,ZZ,XX)%*%q0par ) ) * exp( cbind(1,ZZ,DD,XX)%*%q1par ) * exp( cbind(1,ZZ,MM,DD,sqrt(abs(XX))+3 )%*%q2par ) )
  
  #### proximal hybrid 1 ####
  psiboots_Ph1[j] <<- mean( AA * (cbind(1,WW,DD,sqrt(abs(XX))+3 )%*%h1par) * ( 1+ exp(- cbind(1,ZZ,XX)%*%q0par ) ) )
  
  #### proximal hybrid 2 ####
  psiboots_Ph2[j] <<- mean( (1-AA) * (cbind(1,WW,MM,DD,XX)%*%h2par) * ( 1+ exp(- cbind(1,ZZ,XX)%*%q0par ) ) * exp( cbind(1,ZZ,DD,XX)%*%q1par ) )
  ...
  psiboots_PQR[j] <<- mean( AA * ( 1+ exp(- cbind(1,ZZ,XX)%*%q0par ) ) * ( cbind(1,WW,DD,sqrt(abs(XX))+3 )%*%h1par - cbind(1,WW,sqrt(abs(XX))+3 ) %*% h0par ) 
                            + (1-AA) * ( 1+ exp(- cbind(1,ZZ,XX)%*%q0par ) ) * exp( cbind(1,ZZ,DD,XX)%*%q1par ) * ( cbind(1,WW,MM,DD,XX)%*%h2par - cbind(1,WW,DD,sqrt(abs(XX))+3 )%*%h1par )
                            + AA * ( 1+ exp(- cbind(1,ZZ,XX)%*%q0par ) ) * exp( cbind(1,ZZ,DD,XX)%*%q1par ) * exp( cbind(1,ZZ,MM,DD,sqrt(abs(XX))+3 )%*%q2par ) * ( YY - cbind(1,WW,MM,DD,XX)%*%h2par )
...
for (i in 1:nsimu) {
  ...
  psi_PIPW[i] <- mean( A*Y * ( 1+ exp(- cbind(1,Z,X)%*%q0par ) ) * exp( cbind(1,Z,D,X)%*%q1par ) * exp( cbind(1,Z,M,D,sqrt(abs(X))+3 )%*%q2par ) )
  
  #### proximal hybrid 1 ####
  psi_Ph1[i] <- mean( A * (cbind(1,W,D,sqrt(abs(X))+3 )%*%h1par) * ( 1+ exp(- cbind(1,Z,X)%*%q0par ) ) )
  
  #### proximal hybrid 2 ####
  psi_Ph2[i] <- mean( (1-A) * (cbind(1,W,M,D,X)%*%h2par) * ( 1+ exp(- cbind(1,Z,X)%*%q0par ) ) * exp( cbind(1,Z,D,X)%*%q1par ) )
  
  #### proximal quadruply robust ####
  psi_PQR[i] <- mean( A * ( 1+ exp(- cbind(1,Z,X)%*%q0par ) ) * ( cbind(1,W,D,sqrt(abs(X))+3 )%*%h1par - cbind(1,W,sqrt(abs(X))+3 ) %*% h0par ) 
                      + (1-A) * ( 1+ exp(- cbind(1,Z,X)%*%q0par ) ) * exp( cbind(1,Z,D,X)%*%q1par ) * ( cbind(1,W,M,D,X)%*%h2par - cbind(1,W,D,sqrt(abs(X))+3 )%*%h1par )
                      + A * ( 1+ exp(- cbind(1,Z,X)%*%q0par ) ) * exp( cbind(1,Z,D,X)%*%q1par ) * exp( cbind(1,Z,M,D,sqrt(abs(X))+3 )%*%q2par ) * ( Y - cbind(1, W, M, D, X)%*%h2par )
